...
**Mod 加载失败或运行时报错时，必须第一时间分析官方日志。** 日志中包含 Mod 加载状态、XML 解析错误、XPath 报错、C# 异常堆栈等关键信息，是定位问题的唯一可靠来源。

### 4.2 日志文件位置

七日杀的日志文件位于以下路径（Windows 平台）：

```
C:\Users\<电脑用户名>\AppData\LocalLow\The Fun Pimps\7 Days To Die\
```

**说明**：
- `<电脑用户名>` 需替换为当前 Windows 登录用户名
- `AppData` 是隐藏文件夹，需在文件资源管理器中开启"显示隐藏的项目"才能看到
- 直接在文件资源管理器地址栏粘贴完整路径即可访问

### 4.3 关键日志文件

| 文件名 | 用途 |
|--------|------|
| `output_log.txt` | **主日志文件**，包含 Mod 加载、XML 解析、C# 异常等所有信息 |
| `Player.log` | Unity 引擎日志，UI 渲染相关问题 |
| `Player-prev.log` | 上一次启动的日志（备份） |

### 4.4 开发/调试流程

当 Mod 出现问题时（加载失败、报错、UI 不显示等），AI 必须：

1. **优先分析日志**：首先让用户提供 `output_log.txt` 中相关 Mod 名称的报错段落
2. **匹配关键关键字**：
   - `XML:` — XML 解析错误（标签未闭合、属性错误、XPath 找不到目标）
   - `EXCEPTION:` — C# 异常堆栈
   - `Mod: Loading mod:` — Mod 加载状态
   - `Could not load` / `Failed to load` — 资源加载失败
   - `NullReferenceException` — 空引用异常
   - `MissingMethodException` — 方法签名不匹配（V3.0 API 变更）
3. **根据日志精确定位问题**：
   - 报错在哪个 Mod、哪个 XML 文件、哪个 XPath
   - C# 异常的方法名、类名、行号
4. **修复后让用户重测**：必须让用户重新启动游戏并再次查看日志，确认错误已消除

### 4.5 主动询问原则

当 Mod 出现问题时，AI 应主动询问：

> "请提供 `C:\Users\<用户名>\AppData\LocalLow\The Fun Pimps\7 Days To Die\output_log.txt` 中报错段落的内容，特别是包含 Mod 名称或异常堆栈的部分。"

### 4.6 禁止行为

- 禁止在不查看日志的情况下猜测 Mod 错误原因
- 禁止修改代码后不让用户重新测试就直接下结论"已修复"
- 禁止忽视日志中的警告信息（Warning 往往预示后续崩溃）
